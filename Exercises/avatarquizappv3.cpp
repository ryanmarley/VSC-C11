...
int main ()
{
    ...
    if(userAnswer == 'Q') {
        ...
        write(questions, "questionsOutFile.txt");
        return 0;
    }
}
char printBanner()
...
void questionsFile(vector<Avatar>&questions, char userChoice)
{
    vector<int>character(7);//vector for character count
    for (unsigned int i = 0; i < questions.size(); i++) {
        ...
        if (userChoice == 'a') {
            ...
            cout << "Unknown character, please choose characters a - f" << endl;
            i--;
        }
    }
    int maxValue = maxLoop(character);//returns max value
    int maxIndex = find(character, maxValue);//returns the location of the highest value
    switch(maxIndex) {
        case 0:
            aang();
            break;
...
int maxLoop(vector<int> character) //loops and checks indexes
{
    int maxValue = -1;
